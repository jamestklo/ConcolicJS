...
While there have been numerous publications on concolic testing, Kudzu~\cite{kudzu} and Jalangi~\cite{jalangi} are the only two for concolic testing software that are written in a dynamically typed language; and both focus on JavaScript.  
...
When \tool instruments JavaScript, it would label each constant value as a constant: e.g. the JavaScript statement {\tt var x = "string";} would be instrumented into {\tt var x = \_CONST("string");}.
