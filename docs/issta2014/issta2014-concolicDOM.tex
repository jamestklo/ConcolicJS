...
However, concolic testing Web applications remains a challenge because 
executing different parts of JavaScript code often requires different yet precise background tree structures of the application's Document Object Model.
When a line of DOM related JavaScript code fails, the entire execution can halt and the test case cannot conintue.  
In this paper we present our approach to generating background DOM trees for concolic testing JavaScript Web applications.  
Our approach is designed to be automatic because the number of unique DOM trees can grow exponentially with the number of possible execution paths.
Our approach is also systematic that it can tailor the DOM tree to target precise subsets of the code base.  
We conducted a case study on the DOMtris application in which we will show how our approach significantly improves path coverage that 
includes a part of JavaScript code that implements a core functionality of the application.
\end{abstract}
